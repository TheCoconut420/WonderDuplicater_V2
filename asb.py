...
class ASB:
    ...
    def from_binary(cls, data):
        ...
        assert version in (0x417, 0x418), f"Unsupported version {hex(version)}"
    ...
    def read_node(self, sync_offset):
        ...
        raw_node_type = self.stream.read_u16()
        try:
            node["Node Type"] = NodeType(raw_node_type).name
        except ValueError:
            raise ValueError(
                f"Node-Typ {raw_node_type} (0x{raw_node_type:x}) wird von diesem "
                "Parser noch nicht unterstuetzt (z.B. TwoDimensionalBlender/26 fehlt "
                "aktuell). Bitte diese ASB-Datei nachreichen, damit der Typ ergaenzt "
                "werden kann."
            )
    ...
    def calc_body_size(node, version):
        if node["Node Type"] == "FloatSelector":
            ...
            raise ValueError(
                f"Node-Typ '{node['Node Type']}' (Node Index "
                f"{node.get('Node Index', '?')}) wird von diesem Writer noch "
                "nicht unterstuetzt (z.B. TwoDimensionalBlender fehlt aktuell)."
            )
        if "Body" in node:
    ...
    def to_binary(self, output_dir=""):
        ...
        with open(os.path.join(output_dir, self.filename + ".asb"), "wb") as f:
            ...
            for node in self.nodes:
                try:
                    node_type_value = NodeType[node["Node Type"]].value
                except KeyError:
                    raise ValueError(
                        f"Node-Typ '{node['Node Type']}' (Node Index "
                        f"{node.get('Node Index', '?')}) wird von diesem Writer "
                        "noch nicht unterstuetzt (z.B. TwoDimensionalBlender fehlt "
                        "aktuell)."
                    )
                buffer.write(u16(node_type_value))
                if "Sync Controls" in node:
